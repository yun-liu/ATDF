    ...
    for (int n = 0; n < bn; n++) {
      for (int c = 0; c < bc; c++) {
        for (int h = 0; h < bh; h++) {
          for (int w = 0; w < bw; w++) {
            int new_c = c/(ps*ps);
            int new_h = h*ps + (c%(ps*ps))/ps;
            int new_w = w*ps + (c%(ps*ps))%ps;
            int top_index = ((n*tc + new_c)*th + new_h)*tw + new_w;
  ...
  if (ps != 0) {
    ...
    for (int n = 0; n < tn; n++) {
      for (int c = 0; c < tc; c++) {
        for (int h = 0; h < th; h++) {
          for (int w = 0; w < tw; w++) {
            int old_c = c*ps*ps + (h%ps)*ps + w%ps;
            int old_h = h/ps;
            int old_w = w/ps;
            int bottom_index = ((n*bc + old_c)*bh + old_h)*bw + old_w;
