...
template <typename Dtype>
__global__ void ComputeIndexLabelGPU(const int nthreads, const Dtype* gt_data,
  Dtype* target, const int index) {
    CUDA_KERNEL_LOOP(i, nthreads) {
      int value = static_cast<int>(gt_data[i]);
      target[i] = (value == index) ? 1 : 0;
    }
}
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  const Dtype* target;
  if (this->layer_param_.loss_param().has_label_index()) {
    const Dtype* gt_data = bottom[1]->gpu_data();
    Dtype* label_data = labelmap_.mutable_gpu_data();
    ComputeIndexLabelGPU<<<CAFFE_GET_BLOCKS(count),
      CAFFE_CUDA_NUM_THREADS>>>(count, gt_data, label_data, label_index_);
    target = labelmap_.gpu_data();
  }
  else {
    target = bottom[1]->gpu_data();
  }
