...
__global__ void SigmoidCrossEntropyUnweightedLossForwardGPU(const int nthreads,
...
template <typename Dtype>
__global__ void SigmoidCrossEntropyUnweightedLossNormlizeDiffGPU(const int nthreads,
...
void SigmoidCrossEntropyUnweightedLossLayer<Dtype>::Forward_gpu(
  ...
  Dtype temp_loss = 0;
  for (int i = 0; i < num; i++) {
    SigmoidCrossEntropyUnweightedLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(dim),
        CAFFE_CUDA_NUM_THREADS>>>(dim, input_data + i*dim, target + i*dim,
        bottom_diff + i*dim);
    caffe_gpu_asum(dim, bottom_diff + i*dim, &temp_loss);
    loss += temp_loss;
  }
...
void SigmoidCrossEntropyUnweightedLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    SigmoidCrossEntropyUnweightedLossNormlizeDiffGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
